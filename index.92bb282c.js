...
})({"IIYg2":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "3af5c92d92bb282c";
"use strict";
...
parcelHelpers.export(exports, "Celerity", ()=>(0, _indexDefault.default));
parcelHelpers.export(exports, "Types", ()=>_types);
var _createCustomComponent = require("./createCustomComponent");
var _createCustomComponentDefault = parcelHelpers.interopDefault(_createCustomComponent);
var _index = require("./element/index");
...
const createCustomComponent = (name, config)=>{
    ...
    let shadowMode = "open";
    if (config.shadow) {
        if (config.shadow.useShadow == false) useShadow = false;
        if (config.shadow.shadowMode) shadowMode = config.shadow.shadowMode;
        config.shadow.useShadow = useShadow;
        config.shadow.shadowMode = shadowMode;
    } else config.shadow = {
        useShadow,
        shadowMode
    };
    class CustomComponent extends HTMLElement {
        constructor(){
            ...
            this._initialized = false;
            this._isInitializing = false;
            this[0, _constants.PROPS] = this._initializeProps(this._config);
        }
        getConfig(key) {
     ...
     */ dispatchInternalEvent(name, payload = null, meta = null) {
        ...
        //Dispath EventBusEvent
        /*
     * take the host from the event.target and loop over the eventBus dn check for host, if not make
     * host the CEL_HOST until null
     ...
     */ async dispatchEventBusEvent(name, payload = {}, e) {
            ...
            if ((0, _actions.actionUtils).checkIfActionIsInternal(name)) {
                ...
                internalAction = (0, _actions.actionUtils).getInternalAction(name);
            }
            if (!eventBusEvent || eventBusEvent.length == 0) {
                if (!isInternalAction) console.log(`${name} not in eventBus, something went wrong`);
                return;
            }
            const event = e;
            ...
            if (!event || !event.target) {
                console.error("DispatchEvent should contain Event");
                throw new Error("Missing event in dispatchEvent");
            }
            let host;
            host = event.target;
            //If its internal action then host{CEL_HOST} is not defined
            if (!isInternalAction) host = host[0, _constants.CEL_HOST];
            else host = host._host;
            outerloop: while(host != null || host != undefined){
                let flag = false;
                let index = eventBusEvent.length - 1;
                innerloop: while(index >= 0){
                    ...
                    if (flag) {
                        ...
                        //Seems like window.scheduler is not available in fireefox
                        if ((0, _windowDefault.default).scheduler) await (0, _windowDefault.default).scheduler.postTask(()=>{
                            eventBusEvent[index].handler(payloadWithDetails);
                        }, {
                            priority: "user-visible"
                        });
                        else eventBusEvent[index].handler(payloadWithDetails);
                        //Break the handling of event
                        if ((0, _windowDefault.default).eventBus[name][index].stopPropagation === true || isInternalAction && internalAction && internalAction.stopPropagation == true) break outerloop;
                        else break innerloop;
                    } else flag = false;
                    index--;
                }
                if (isInternalAction) //We did not find the event registered for this host and as its an internalEvent
                // host will only be one, so should exit
                host = null;
                else if (!host[0, _constants.CEL_HOST]) host = null;
                else host = host[0, _constants.CEL_HOST];
            }
        }
        /*
     * InitialzeProps
     */ _initializeProps(config) {
        ...
        _reconcileProps() {
            const props = this[0, _constants.PROPS];
            //reconcile props from the props attribute to props to pass into view
            for(const key in props){
                if (this.getAttribute(key)) props[key] = this.getAttribute(key);
                else if (key in this) props[key] = this[key];
            }
            this[0, _constants.PROPS] = {
                ...props
            };
        }
        getState() {
            return ()=>{
                return this[0, _constants.STATE];
            };
        }
        /*
    ...
    */ connectedCallback() {
            ...
            for(const key in actionHandlers){
                ...
                (0, _windowDefault.default).eventBus[actionHandlers[key].name].push({
                    handler: (0, _actions.eventBusHandlerFactory)({
                        getState: this.getState.bind(this).apply(this),
                        updateState: this.updateState.bind(this)
                    }, actionHandlers[key].handler, this[0, _constants.PROPS], this),
        ...
        disconnectedCallback() {
            //Dispath lifecycle action
            this.dispatchEventBusEvent((0, _actions.internalActions).CEL_COMPONENET_DISCONNECTED, {}, {
                target: this
            });
        }
        ...
        _render() {
            //reconcile props from the props attribute to props to pass into view
            if (this._initialized) this._reconcileProps();
            //dispatchInternalEvent not in use (CustomEvent method)
            const data = {
                props: this[0, _constants.PROPS],
                state: this[0, _constants.STATE],
                updateState: this.updateState.bind(this),
                dispatchDomEvent: this.dispatchInternalEvent.bind(this),
                dispatch: this.dispatchEventBusEvent.bind(this),
                host: this
            };
            let view;
...
const createComponent = (name, config)=>{
    customElements.define(name, createCustomComponent(name, config));
};
...
parcelHelpers.export(exports, "CEL_HOST", ()=>CEL_HOST);
const BROWSER_IDLE_TIMEOUT = 1000;
...
function generateRandomData(numPoints) {
    ...
    for(let i = 0; i < numPoints; i++){
        let point = {
            x: Math.random() * 100,
            // generate a random x value between 0 and 100
            y: Math.random() * 100 // generate a random y value between 0 and 100
        };
        data.push(point);
    }
    return data;
...
const renderPlot = (numPoints, chart)=>{
    ...
    const svg = chart.attr("width", width).attr("height", height);
...
(0, _core.Component)("main-app", {
    ...
    props: {},
    template: App,
    styles: (0, _appCssDefault.default),
    handlers: [
        {
            name: "CEL#COMPONENT_CONNECTED",
